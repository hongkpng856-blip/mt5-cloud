...
#property strict
#property description "Test EA (breakout style): Open every 25s, hold 18s."

//--- input parameters
input double InpLotSize      = 0.10;    // 手數
input int    InpIntervalSec  = 25;      // 開單間隔(秒)
input int    InpHoldSec      = 18;      // 持倉時間(秒)
input int    InpSlippage     = 30;      // 最大滑點(點)
input int    InpMagic        = 240705;  // Magic Number
...
int OnInit()
{
   ...
   WriteStats();
   Comment("TestBreakout EA 已啟動 — 每 " + IntegerToString(InpIntervalSec) + " 秒開一單，持倉 " + IntegerToString(InpHoldSec) + " 秒平倉");
   Print("TestBreakout EA 啟動: ", Symbol(), " lot=", DoubleToString(InpLotSize, 2), " magic=", InpMagic);
...
void OnDeinit(const int reason)
{
   ...
   CloseAllPositions();
   Comment("");
   Print("TestBreakout EA 停止 (reason=", reason, ") 總交易=", g_cycle, " 買=", g_buy_count, " 賣=", g_sell_count);
}
...
void OpenTestOrder()
{
   ...
   if(result.retcode == TRADE_RETCODE_DONE || result.retcode == TRADE_RETCODE_PLACED)
   {
      ...
      Print("📈 TestBreakout 開單 #", result.order, " ", (isBuy ? "Buy" : "Sell"), " ", _Symbol, " ", DoubleToString(lot, 2), " @ ", DoubleToString(price, _Digits), " (cycle=", g_cycle, ")");
      UpdateComment();
   }
   else
   {
...
void ClosePosition(ulong ticket)
{
   ...
   request.deviation    = InpSlippage;
   request.magic        = InpMagic;
   request.comment      = "TestBreakout close";
   ...
   if(result.retcode == TRADE_RETCODE_DONE || result.retcode == TRADE_RETCODE_PLACED)
   {
      ...
      Print("📉 TestBreakout 平倉 #", ticket, " profit=", DoubleToString(closed_profit, 2), " 累計=", DoubleToString(g_total_profit, 2));
      UpdateComment();
      WriteStats();
...
//+------------------------------------------------------------------+
// 寫統計落 state_TestBreakout.json（detector 讀呢個檔判斷運行狀態 + 我加 stats）
// 路徑：Common/Files/state_<EA>.json
// 🚨 2026-08-21：加 win_sum/loss_sum（贏嘅總額/輸嘅總額 — 準確計 avg win/loss）
void WriteStats()
{
   string fname = "state_TestBreakout.json";
   int wins = 0, losses = 0;
   double win_sum = 0, loss_sum = 0;
   ScanHistoryStats(wins, losses, win_sum, loss_sum);
   string json = StringFormat("{\"ea\":\"TestBreakout\",\"status\":\"running\",\"ts\":%I64d,\"trades\":%d,\"wins\":%d,\"losses\":%d,\"profit\":%.2f,\"win_sum\":%.2f,\"loss_sum\":%.2f}",
