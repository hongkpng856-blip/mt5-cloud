...
| **v0.10.78** | 2026-08-26 | 🔑 **行內人 Magic 做法（用戶要求）：Magic = EA 固定身份 — 一生唔變** — ①**首次部署**：EA 冇分配 → 自動分配未用固定 magic（240701、240702…）+ 記錄入 `config['_magic_assignments']`（`_` 開頭 — DELETE 唔會清 — 持久保留）②**剷除再部署**：沿用返舊 magic（歷史連貫 — Correlation/統計完整串返 — 唔會因為重新部署而 magic 變咗斬斷歷史）③**用戶指定特別 magic**（777/888 等）→ 尊重用戶 + 記錄 — 實測：ATR_Stop 首次 240704 → 剷除 → 再部署沿用 240704（`[deploy] 🔑 ATR_Stop 沿用固定 Magic 240704（歷史連貫）`）|
...
| 121 | **🔑 剷除再部署 magic 變咗（斬斷歷史 — 行內人會用返同一個 magic）** | v0.10.77 撞 magic 就 +1（每次部署可能唔同 magic）→ 剷除再部署 magic 變 → 舊交易+新交易分開計（Correlation/統計唔連貫）| v0.10.78 行內人做法：`config['_magic_assignments']` 持久分配表（`_` 開頭 DELETE 唔清）— 首次部署分配固定 magic + 記錄；剷除再部署沿用返舊 magic（歷史連貫）；用戶指定 magic 尊重。實測：ATR_Stop 240704 剷除→再部署沿用 | 08-26 |
...
**Git HEAD**: `3972c09`（master）— v0.10.78（行內人 Magic 做法：首次部署分配固定 magic 入持久分配表 + 剷除再部署沿用舊 magic）；TODO：數據注入選擇功能未實行（見 TODO 段）
