...
| # | 描述 | 狀態 | 影響 |
|---|---|---|---|
| #148 | **Server 靜默 crash（install-local 期間）** — `pythonw.exe MSVCP140.dll 0xc0000005`（底層記憶體違規 — eventlet/threading 併發） | 🔴 待根治 | 用戶添加/部署突然冇反應 — 要手動重啟 server（加 watchdog 自動重啟 or 查 eventlet 資源洩漏） |
| #154 | **剷除流程漏清** — TradotcomAgent 目錄仲有舊嘢（agent_launcher.log、paint_uimap、舊 Setup 檔） | ⏳ 待清 | 安裝目錄殘留舊檔案 |
| #156 | **網頁 runtime 狀態顯示滯後**（部署完成但顯示「圖表移除」— 要 refresh 先正確） | ✅ 已修（v0.11.02 — 操作完成即刻 refresh 配對庫） | — |
| #157 | **剷除誤傷其他 EA**（重複 pause_cmd + fallback 亂關 chart — Multi_TimeFrame 案例） | ✅ 已修（v0.11.03 — pause dedupe + 唔亂關 chart） | — |
