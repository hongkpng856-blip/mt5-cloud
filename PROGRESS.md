...
| # | 描述 | 狀態 | 影響 |
|---|---|---|---|
| 173 | **企業級自動更新機制（41628e4）+ 剷除 Agent bug（88e82d5）** — ①**自動更新**：agent.py 加 `AGENT_VERSION` + server `/api/agent-version` + agent `_check_agent_update`（啟動 15 秒後 check — 有新版 → tkinter 彈視窗通知 → 用戶確認 → 下載更新 + 重啟）— **注意：B 機（claw — 路徑 `C:\Users\claw\AppData\Local\TradotcomAgent`）而家行嘅 agent.py 已係最新**（direct_launch 每次自動下載 — 有自動更新 code → 下次有新版彈視窗）②**剷除 Agent bug（88e82d5）**：`_remove_agent` 標記殘留喺 deploy_queue（agent 收唔到 shutdown/未完成 → 標記永遠留低）→ 新 agent 啟動後 poll 讀到 → **又剷自己**（惡性循環 — B 機剷除後再安裝：config 冇刪 → launcher 讀舊 config → 直接啟動跳過安裝精靈）→ 修復：remove-complete + 重新註冊時清 `_remove_agent` 標記（實測：B 機標記已清 + connected ✅）③**offline 依然見到 balance（21b8874）**：api_dashboard `_acc_dis` 唔理 status → offline 都顯示舊 balance → 改 offline 清空 account ④**待測（用戶 mark 低）：剷除後再安裝 → 安裝精靈應該彈返**（完整流程 — 剷除正常清 config → 再安裝彈精靈）— 而家 B 機 agent online（未測）⑤**B 機（claw）部署/加入排查 — 未完成（09-06 mark 低）**：a) B 機 agent 啟動用 DEV00001/tradotcom.com（唔係 config 0121e9f0/直連 IP）— agent_config.json 已改最新但 agent 唔讀（pyw/launcher 唔喺 TradotcomAgent 目錄 — 未知啟動源 — 待查）b) watcher log 顯示 `Server: https://tradotcom.com + Agent ID: DEV00001`（舊 watcher 殘留 — 用舊 config）c) `No module named 'pyautogui'`（已裝 — 待確認）+ `No module named 'win32api'`（pywinauto/win32 依賴 — 待裝/確認）d) 前端「加入/剷除都顯示 Deploy」steps（server 可能 deploy steps 覆蓋 pairing steps — 待查）e) 警告視窗卡死（Verify running status doing + fail status 唔識顯示 — 前端只 check 'failed' 唔 check 'fail'）f) agent.py 加咗「冇參數自動讀 agent_config.json」fallback（ba2a7cc — 待 B 機實測）| ✅ 已修（④精靈重裝待測） | B 機 agent 已最新 + 標記已清 + online — 剷除→安裝精靈流程待用戶測試 |
| 172 | **配對/部署/鏟除假成功 + 心跳顯示 + 網頁版步驟同步 + 清空白按鈕（09-04 系列）** — ①**網頁版警告視窗 steps 同步（已修）**：agent control_steps 上報 server 冇 work（server log：控制 steps 空/冇）→ **root cause：agent 行緊舊 code** → 重啟（最新版）→ 上報正常（VPS steps 由 agent 上報覆蓋 — 網頁同步 ✅）②**清理空白圖表按鈕（網頁版）已修 4 個**：a) 網頁版冇警告視窗 — cleanBlankCharts 漏咗 showControlModal（d286448）b) modal 空白 — showControlModal 喺 server steps 寫好前彈 → steps mtime 舊 → API 成功後先彈（75b8037）c) 5 秒 auto reload 打斷 — 改延遲 refresh 由 pollAiControl 控制（d03db74）d) 電腦版卡最尾 — `_update_steps` 累積殘留 → 直接覆寫雙寫（7d97902）③**按鈕「清理中」唔復原（已修 — VPS restart 後實測）**：allDone 復原（dcd84c5）+ 獨立輪詢復原（f870660 — 每 3 秒 poll steps 全 done 復原）— **用戶實測 VPS restart 後按鈕有復原 ✅**④**配對庫唔顯示 script（5920935）**：local_eas 只掃 Experts（唔掃 Scripts）⑤**配對庫心跳冇顯示（d30537d）**：check_ea_heartbeat_files 只掃 `hb_.txt` — EA 寫 `state_*.json` → 全部 miss → 加掃 state_*.json（TestStoch/TestRSI 心跳正常顯示）⑥**斷線循環真正 root cause（0b40e8e）**：`sio.connect(wait=False)` 冇處理 engine.io ping/pong → server 60s timeout 斷 → agent 每 ~68 秒斷線（install/deploy 喺斷線窗口中斷假失敗）→ 加 `sio.wait()` keep-alive thread ⑦**配對假成功（b869206）**：舊 .ex5 存在就 skip compile → 心跳碼冇注入 → 改 `.ex5 新過 .mq5 先 skip`（.mq5 新 → 重新 compile）⑧**部署卡死（22f1235）**：`.ai_control.lock` 殘留（deploy TestMACD|PID — auto_attach timeout 死咗 lock 冇清）→ watcher 永遠 skip deploy（「部署又有問題」「冇開番 MT5」）→ lock PID 驗證 + 殘留清理 ⑨**鏟除後 .chr 殘留（f444fec）**：fallback Ctrl+W 關 chart 但冇刪 .chr → MT5 restart restore 空白 → fallback 成功後刪 .chr — **人手測試 PASS（09-04）**：配對 TestBreakout（新鮮 .ex5 19584b）→ 部署（心跳 10s + MT5 log loaded）→ 鏟除（心跳+.mq5+.chr 全刪）— TestTrend/TestScalper 同系列 PASS — 配對庫最後：TestMACD/TestRSI/TestStoch running（心跳正常）| ✅ 已修 | 配對/部署/鏟除/心跳全鏈根治（人手測試 PASS）— 網頁版 steps 同步 + 清空白按鈕復原全部已修 |
| 171 | **VPS 搬遷 — 配對/剷除卡住 + 各項遠端化修復（方案2 核心工程）** — server（VPS）冇 MT5 — 配對（install-local）原本 server 自己複製/編譯去 VPS（冇 MT5 → 卡）；剷除（remove-local）同樣卡。**已修（一連串 commit）**：①**配對+剷除遠端化（d5d9eb8）**：server 偵測 agent connected → 發 `install_ea_command`/`ea_remove_command`（SocketIO）→ agent 自己機下載+安裝+編譯 / 刪檔案+寫 pause_cmd（watcher remove chart）②**EA庫 download 404（03a0c41）**：EA_LIBRARY_DIR 等相對路徑 → abspath（新 werkzeug safe_join 拒 `..`）③**EA庫 local_has 用 agent snapshot（ba313a6）**：agent `build_files_snapshot` 加 local_eas 掃描（Experts/Scripts .mq5/.ex5）→ server 唔再讀 VPS 檔案 ④**Steps+Symbols 上報（39d08d8）**：agent sync 帶 control_steps（本地 .ai_control.steps）+ symbols（過濾後）→ server 寫自己 steps（網頁同步）+ symbol dropdown ⑤**Symbols 過濾（56125f8）**：12455（全部市場 incl 股票）→ 58（FX/金屬/指數）⑥**Agent 斷線循環（1b5ba0f）**：reconnect 全改 polling（VPS eventlet websocket 唔穩定 — 每 ~50 秒斷）⑦**配對卡住（c14794c）**：agent 完成後寫 steps done + 清 show flag → 網頁唔再卡「進行中」⑧**Compile 卡死（6a2205e → f70188f）**：compile 前 kill 舊 MetaEditor + subprocess.run 改 Popen + 輪詢 .ex5 — **09-03 下晝加碼（假成功真正根治 — 用戶實測多輪）**：⑨**停用註冊 auto-send bulk install（10110c0）**：agent 每次註冊/重連 auto-push 全部 EA（ea_name=all — bulk install background）同用戶手動配對（install-local 單獨）撞 → compile 衝突 → 假成功 ⑩**download URL 域名→直連 IP（142f9bc）**：server 發嘅 download_url 用 request.host_url（tradotcom.com — 經 CF 慢/斷）→ agent 改用自己 SERVER_URL（直連 116.206.150.233）⑪**Navigator cooldown 擋 refresh（fd41975）**：_refresh_cooldown 300 秒擋咗「新增 .ex5」嘅 refresh（compile 多次 dir 變化 → 第一次 refresh 太早 → 最後 .ex5 生成冇 refresh）→ 新增檔案唔受 cooldown ⑫**install SocketIO handler thread 中斷（2eea910）**：download_and_install 喺 handler thread 行 — socket 斷線（每 ~30-60s）→ 中斷 compile → 改 background daemon thread ⑬**雙 install race（38429c8 + 001450f）**：emit + poll fallback 同時 → 兩個 install → 兩個 MetaEditor → compile 雙失敗 → thread lock + `_install_in_progress` set ⑭**同 ID 多開（320eb45）**：agent.lock 機制「同 agent_id → 允許」→ respawn 嘅同 ID agent 全部起到（實測 11 個 agent 同時行！互相 kill MetaEditor/覆蓋 flag → 一切假成功根源）→ 改 check PID（lock PID 仲行緊 → 阻止）⑮**存檔 double CR（ac1dc3e）**：`newline='\r\n'` + content 已 normalize → CRCRLF → MetaEditor parser 讀唔到（實測 364 CRCRLF）→ 改 `newline=''` ⑯**MetaEditor /compile 格式（2f2bbf6）**：**真正 root cause** — MetaEditor CLI 要 `/compile:path`（冒號格式）— agent 用 `/compile path`（分開）→ MetaEditor 唔識處理 → 卡 60 秒 timeout → 假失敗 → 改 `/compile:{path}`（實測 2 秒 compile 成功 0 errors）⑰**配對完成延遲 refresh（ed60484）**：compile 完成 → 等 4 秒（MT5 detect 新 .ex5）→ refresh Navigator（watcher refresh 太早）— **用戶實測 PASS：配對成功 + Navigator 自動 refresh（唔使手動）** | ✅ 已修（按鈕復原未收好） | 配對/部署/鏟除/心跳全鏈根治（人手測試 PASS）— 清空白按鈕復原待重測 |
| 170 | **Git watcher 自動 restart server 閃退（已修 — 改用 restart bat）** — git_auto_update.py watcher Popen 開 server 喺 VPS 閃退（10061）— **已修**：改用 `restart_git_server.bat`（git pull + taskkill + PowerShell 開 server PORT=80 + 驗證 — double-click 一鍵）— watcher 全自動更新未完成（暫停 — 手動 double-click restart bat 代替） | 🟡 部分修 | VPS 更新 = double-click restart_git_server.bat（git pull + 重啟） |
| 169 | **B 電腦安裝 Agent 用錯身份（fef654c3 — 舊 config 殘留）** — 重新註冊新 account（hongkpng858 — 派 0121e9f0）但安裝 Agent 時 pyw「發現 agent_id → 直接啟動模式」跳過安裝精靈 → 用舊 config（fef654c3 — 唔屬於任何 account）→ server 靜默（agent 唔存在冇回覆）→ 網頁 Offline + 冇 balance — **已修（fb27811）**：①server handle_register agent 唔存在 → 回覆 `unknown_agent` error ②agent on_registered 收到 `unknown_agent` → 自動清 agent_config.json + lock + 退出 → 下次開 launcher 彈安裝精靈（用戶重新填正確 ID） | ✅ 已修 | 剷除 Agent / 錯 ID 唔會再殘留 — 下次安裝一定彈精靈 |
| 168 | **VPS 搬遷（進行中 → 大部分完成）** — server 由本機搬去 VPS（116.206.150.233 — Windows Server 2012 R2 — 1 CPU/2GB）— ✅ 已做：①VPS 裝 Python 3.8.10（3.11 唔支援 Server 2012 — stopped working — 裝 VC++ Runtime 解決）②server code + DB 搬過去③pip 裝依賴④server 啟動（PORT=80）⑤防火牆開 80⑥Cloudflare DNS：CNAME tunnel → A record 指 VPS IP + SSL Flexible⑦agent 直接 IP 連 VPS 穩定 — **VPS 上發現+修復：**⑧agent-download 404（新版 werkzeug safe_join 拒絕相對 `..` 路徑 → 改 abspath 絕對路徑 — bae04b9）⑨舊網址 esgov → tradotcom.com（agent.py/pyw/launcher/install_agent — efc5236）⑩agent 依賴下載 localhost:5001 → SERVER_URL（B/C/D 電腦 10061 — 162e742）⑪agent Popen 服務要傳 MT5_CLOUD_URL env（162e742）⑫vps_update.py 啟動 server 3 次修復（cmd → PS Start-Process → Python Popen — PS 5.1 唔支援 -Environment — 2471c00）⑬剷除 Agent 殘留（fb27811 — 見 #169）⑭**Git 全自動更新（b082137 + af9008c）**：VPS clone repo（tradotcom folder）+ git_auto_update.py watcher（每 60 秒 check GitHub — 新 commit 自動 pull + 重啟 server）— **驗證：A 電腦（cbb18292）+ B 電腦（0121e9f0/hongkpng858）都 Online + balance 正常** | 🟡 大部分完成 | VPS 搬遷完成（A+B 電腦正常 + Git 全自動更新）— 待：vps_update.py 自動啟動再驗證 |
| 167 | **部署失敗（order.wnd 唔存在 + WM_CLOSE 關 MT5 唔生效 + magic 亂 — 3 個 root cause）** — ①**order.wnd 唔存在就 skip**（MT5 開住時 order.wnd 喺 memory — 唔存在 → code skip → 開機唔 restore chart → EA 唔掛）— **已修（9c232aa）**：order.wnd 唔存在都建立 ②**WM_CLOSE 關 MT5 唔生效**（MT5 有 dialog/狀態擋住 → 冇 restart → 唔 restore）— **已修（3d408a1）**：WM_CLOSE 後檢查 — 未關就 taskkill /F ③**install-local magic 亂**（config.setdefault magic='240701' 冇讀 EA 內部 InpMagic → 全部 EA 配對時 magic 錯 + `_magic_assignments` 持久表錯值）— **已修（fb2d3ea）**：`_ea_magic_from_source()` 掃 .mq5 攞 InpMagic + DB 修正 — **實測：部署 TestRSI→GBPUSD 15 秒完成（心跳 FRESH + steps done）** | ✅ 已修 | 部署正常（15-30 秒 — 唔會再因環境狀態失敗） |
| 166 | **警告視窗 steps 卡住（第一行 doing 殘留 — server/agent steps text 唔一致）** — 部署完成（MT5 已掛）但警告視窗 steps 第一行（`deploy X（SYM）`）卡住 doing — server 寫 `deploy X（SYM）`（中文括號+小楷）vs agent 寫 `Deploy X (SYM)`（英文括號+大楷）→ `_update_steps` 按 text 完全相等 merge → merge 唔到 → 原始 doing 行殘留 — **已修（commit b40e074）**：統一全部 steps text（`Deploy {ea_name} ({symbol})` — 英文括號+大楷 — server 4120 + agent 1729/1730-1732） | ✅ 已修 | 警告視窗 steps 全部 done（冇卡住） |
| 165 | **部署「唔掛 EA」誤判（真正原因：.ex5 唔存在 — 唔係格式問題）** — 之前測試部署用 Scalping_M1（.ex5 被剷除時刪咗）→ MT5 讀 .chr 見到 `path=Experts\Scalping_M1.ex5` → 搵唔到檔 → 靜默唔掛 → 誤判「模板格式問題」；**真正 root cause：EA 檔（.ex5）必須存在**（user實測：先配對（compile .ex5）→ 再部署 — 一定有 .ex5 → 成功）— **已解決（commit 2da5135）**：deploy_ea_via_chr 用模板（有 InpSymbol=）+ 實測 Breakout（.ex5 存在）→ 掛到 | ✅ 已解決 | 部署前確保 .ex5 存在（配對流程會生成） |
| 164 | **重新開啟 MT5 開好多視窗（空白 .chr 越積越多 + order.wnd 未同步）** — ①`deploy_ea_via_chr` 冇 call `_clean_blank_charts_via_chr`（清空白 chart）→ 空白 .chr（chart02/06）越積越多 → order.wnd 越積越多 → MT5 開機全開 ②`_clean_blank_charts_via_chr` 刪 .chr 但冇同步 order.wnd → MT5 見 order.wnd 有已刪 chart → 重新生成空白 — **已修（commit 9078c28）**：①deploy 前 call 清空白 ②清完同步 order.wnd（移除已刪項目）— **實測：MT5 重開 chart 7 個 → 5 個（全部有 EA — 空白冇咗）** | ✅ 已修 | 重新開啟 MT5 唔會再開好多空白視窗 |
...
| 163 | **`verify_heartbeat` 讀錯心跳檔（hb_.txt vs state_.json）→ 部署每次等 90s timeout** — EA 寫 `state_<EA>.json`（唔係 `hb_<EA>.txt`）→ verify_heartbeat 只 check hb_.txt → 永遠 check 唔到 → 等足 timeout 先 fallback log check → 部署每次拖慢 90 秒（user實測：部署 100-120 秒太慢）— **已修（commit 78d8f63）**：兩個都 check（state_.json 優先 + hb_.txt 後備）— **實測：部署 120s → 14s（快 8 倍 — verify 即刻 PASS）** | ✅ 已修 | 部署由 120 秒 → 14 秒 |
| 162 | **`remove_ea_via_chr` 話「冇 .chr 檔」→ 剷除 fail（EA 停咗但 chart 冇移除）** — **真正 root cause（debug 揭曉）**：`remove_ea_via_chr` 開頭**冇 `import re`** → `re.search` NameError → try/except 食咗 → 每個 .chr 讀 fail → 話「冇 .chr 檔」（user實測：人手部署 EA 落 chart → .chr 即刻有記錄 — .chr 可靠 — 係 code 讀唔到）— **已修（commit 7f1a1e0）**：①`import re` ②等 .chr 檔出現（最多 30 秒 — MT5 關閉 save 大檔慢）③regex `[^\\r\\n]` → `[^<]`（可靠 match EA 名）— **實測 PASS**：搵到 chart02（Scalping_M1）→ 刪 .chr → 刪 order.wnd → 刪心跳 → 開 MT5 → **USDJPY chart 成個冇咗（剩 2 個）** | ✅ 已修 | 剷除完整成功（EA 停 + chart 成個移除） |
...
> 🎉 2026-09-01：#152/#156/#157/#160/#161/#162/#163（dropdown lag/部署第二隻/代替 dialog/剷除誤剷/冇操作開 MT5 真正 root cause/reconnect 循環/剷除「冇 .chr 檔」真正 root cause/部署慢 90s — verify 讀錯心跳檔）已修（見版本表 v0.11.05/v0.11.06）。
...
| **v0.11.06** | 2026-09-01 | 🔥 **部署機制轉換：用 .chr 方法取代 GUI automation（user實測 + 實驗驗證 + 完整流程測試）** — ①**核心概念（user實測）**：MT5 開機 restore .chr 檔（`MQL5/Profiles/Charts/<profile>/*.chr`）— 自動開 chart + 掛 EA（人手 duplicate chart01→chart03 — 開機見到兩個掛 EA 嘅 chart）；**EA 檔（.ex5）必須存在**（MQL5/Experts/ — 冇就靜默唔掛）②**新部署方法（commit 511dcc4 — deploy_ea_via_chr）**：複製現有 MT5 寫嘅 .chr（有完整 expert 區）→ 改 id/symbol/description/EA 名/path/Magic → 寫入新 chartXX.chr + 更新 order.wnd → 關 MT5 → 開 MT5（restore 自動掛 EA）→ **平鋪窗口（WM_COMMAND id=33527 — 取代改座標 — 避免重疊）** — 取代熱鍵預載（restart ×2）+ Alt+F 開 chart + Ctrl+1 attach（GUI automation — 所有 bug 源頭）— 保留 UAC 檢查 + dialog 清理 + verify_heartbeat 驗證 ③**FIX deploy_ea_via_chr 兩個 bug（commit b71588e）**：`import random` 缺失（NameError）+ path replacement f-string `\\R` escape error（改用 lambda replacement）④**完整流程實測 PASS（添加 → 部署 → 剷除）**：部署 RSI_Over→USDJPY（.chr 寫入 + restart + restore 自動掛 + `RSI_Over (USDJPY,H1) loaded successfully` + 心跳 FRESH + OnInit「已啟動」+ 平鋪）→ 剷除 RSI_Over（刪 .chr + order.wnd + 心跳 + **USDJPY chart 成個冇咗**）⑤**Navigator refresh 實測（user要求）**：導航 toggle（id=32845）**唔 refresh**（只開關面板 — 新 EA 唔出現）；right-click → 刷新（watcher 現有方法 — (271,682)）先真正 refresh — 保留舊方法 ⑥**已知問題（已修）**：`verify_heartbeat` 喺 deploy_ea_via_chr 內 timing 問題（EA 實際掛咗但 verify 話「未確認」— 因為 restart 後心跳寫入慢 — 90s timeout 後先寫）— 唔影響實際結果（EA 真掛）但 steps/watcher 顯示慢 — 已修（見 ⑦）⑦**FIX 部署每次等 90s timeout（commit 78d8f63 — user實測：部署 100-120 秒太慢）**：root cause — `verify_heartbeat` **只 check `hb_<EA>.txt`** 但 EA 而家寫 **`state_<EA>.json`**（唔係 hb_.txt）→ 永遠 check 唔到 → 等足 timeout 先 fallback log check → 每次部署拖慢 90 秒 — 修復：兩個都 check（state_.json 優先 + hb_.txt 後備）— **實測：部署 120s → 14s（快 8 倍 — verify 即刻 PASS `[HB] Scalping_M1 heartbeat ... (14s ago)`）** ⑧**綜合壓力測試 PASS（輕量 — ADD=2 + DEPLOY=2 + REMOVE=2，1 輪 — 人手模擬 + 雙重驗證）**：添加 Bollinger_Band + ADX_Trend（.ex5 生成 + compile 成功）→ 部署 ADX→GBPUSD + Bollinger→USDJPY（loaded + 心跳 FRESH + 網頁 running 一致）→ 驗證（3 EA 心跳全 FRESH）→ 剷除 ADX + Bollinger（心跳刪 + .chr 刪 + chart 成個冇 + 配對庫移除）— **全部 6 步 PASS — 冇誤傷（Scalping_M1 保留）** — cap 圖：screenshots/stress-deploy-2-ea.png + stress-test-complete.png ⑨**FIX 重新開啟 MT5 開好多視窗（commit 9078c28 — user實測）**：root cause — ①`deploy_ea_via_chr` 冇 call `_clean_blank_charts_via_chr`（清空白 chart）→ 空白 .chr 越積越多 → order.wnd 越積越多 → MT5 開機全開 ②`_clean_blank_charts_via_chr` 刪 .chr 但冇同步 order.wnd → MT5 見 order.wnd 有已刪 chart → 重新生成空白 — 修復：①deploy 前 call 清空白 ②清完同步 order.wnd（移除已刪項目）— **實測：MT5 重開 chart 7 個 → 5 個（全部有 EA — 空白冇咗）** ⑩**.chr 模板部署方法確立（commit 2da5135 — user實測 + 實驗驗證）**：**user實測** — 手寫 .chr（有齊欄位 + `<expert>` 區 + **`InpSymbol=`**）放返 Euro folder → 開 MT5 → **自動開 chart + 掛 EA** ✅；**真正 root cause（「唔掛」誤判）** — 之前測試用 Scalping_M1（.ex5 被剷除刪咗）→ MT5 搵唔到檔 → 靜默唔掛 → 誤判「格式問題」；**實際係 .ex5 必須存在**（正常流程：先配對 compile .ex5 → 再部署 — 一定有）；**code 改動** — deploy_ea_via_chr：①Step 1 搵基底加 `InpSymbol=` 條件 ②冇 → 用模板（agent/chr_template_base.chr.txt — user 實測格式）轉 chart_base.chr — **實測：Breakout（.ex5 存在）→ GBPUSD + AUDUSD 都掛到（loaded + 心跳 FRESH）**；模板存檔：agent/chr_template_base.chr.txt + chr_template_README.md（commit 5d22664） ⑪**FIX 部署 NameError + 警告視窗 steps 卡住（commit b6b3475 + d3f67d2 + 892187a + b40e074 — user實測 Server to Agent 完整流程）**：①**NameError（b6b3475）**：`ensure_auto_trading_on(mt5_pid)` — mt5_pid 未定義 → 部署 crash exit 1（但 .chr 已寫 — 部署實際成功）→ 改 `find_mt5_pid()` ②**steps 冇寫 done（d3f67d2）**：deploy_ea_via_chr（新 .chr 方法）成功後冇寫 steps done → 警告視窗卡住 → 加 `_update_steps`（全部 done）③**alert_worker TclError crash（892187a）**：`root._done_btn.pack_forget()` 但 root 已銷毀 → TclError crash → 警告視窗殘留 → 加 `winfo_exists()` 檢查 ④**steps text 唔一致（b40e074）**：server 寫 `deploy X（SYM）`（中文括號）vs agent 寫 `Deploy X (SYM)`（英文括號）→ `_update_steps` merge 唔到 → 第一行 doing 殘留 → 統一全部 text（英文括號+大楷）— **實測：Server to Agent 部署完整流程 PASS（部署成功 + 警告視窗 steps 全部 done — 冇卡住）** ⭐**Server to Agent 測試最穩定版本（user確認）** ⑫**新增「清理空白圖表」功能（commit ec44523 + b3c90b4 + 192d55b — user要求）**：網頁「我的配對庫」card 加「清理空白圖表」按鈕 — 清空 MT5 空白冇部署 EA 嘅 chart（關 MT5 → 清空白 .chr 移去 _deleted + order.wnd 同步 → 開 MT5 → 平鋪 → steps done）— **實測 PASS：清走 3 個空白 chart（USDGEL + USDJPY×2）+ 保留有 EA 嘅（AMD TestTrades — 心跳 FRESH）**；修復過程：①JS event.target bug（onclick 直接 call — event undefined → 冇反應 → 改 this 參數）②警告視窗唔彈（server 寫開發目錄 vs alert_worker 讀安裝目錄 → 雙寫）③clean_cmd 被誤判舊殘留（fingerprint 只比 account 部分 — server 寫 account:X vs config account:X|agent:Y → 拆 | 比較）⑬**「移去配對」改「加入配對庫」（commit 30e7977 — user要求）**：按鈕文字統一（6 處 — 按鈕 + 提示 + 註解）⑭**EA 倉庫改 10 隻 testable EA（commit 0ea1ce1 + 5ae205f — user要求：全部改成可測試 — 不停有數據）**：10 隻唔同名（TestTrades/TestScalper/TestGrid/TestTrend/TestBreakout/TestMeanRev/TestMomentum/TestRSI/TestMACD/TestStoch — magic 240701-240710 + 唔同開單間隔 5-50s）— 每隻 TestTrades 模板（不停開單→平倉 → 真實 Trades/Win/P&L → 心跳 + state json → 網頁即時更新）；刪走 40 隻舊 EA + 社群 EA；**實測 PASS：部署 TestScalper → 心跳 FRESH + 2087 單（trades/wins/losses/profit — 不停有數據）**；**發現 2 個坑**：①「清殘留」誤刪新 EA（`_SYSTEM_KEEP` 唔包 → 動態加 EA 倉庫 .mq5 base）②手動複製 .mq5 會被 MetaEditor 刪（要用「加入配對庫」install-local — download + compile + config — MetaEditor 識）⑮**FIX 部署失敗①（commit 9c232aa）**：order.wnd 唔存在就 skip（MT5 開住時 order.wnd 喺 memory — 唔存在 → 開機唔 restore chart → EA 唔掛）→ 唔存在都建立 ⑯**FIX 部署失敗②（commit 3d408a1 — user實測：WM_CLOSE 關唔到 MT5）**：WM_CLOSE（PostMessage 0x0010）被 dialog/狀態擋住 → MT5 冇 restart → 唔 restore → EA 唔掛 → WM_CLOSE 後檢查未關就 taskkill /F ⑰**FIX install-local magic 亂（commit fb2d3ea — user實測：配對庫 magic 全部錯）**：config.setdefault magic='240701' 冇讀 EA 內部 InpMagic → 全部 EA 配對時 magic 錯（TestTrades=240702/TestScalper=240703 等）+ `_magic_assignments` 持久表錯值 → `_ea_magic_from_source()` 掃 .mq5 攞 InpMagic + DB 修正 — **實測：部署 TestRSI→GBPUSD 15 秒完成（心跳 FRESH + steps done — 3 隻 EA 同時運行）** |
...
| **v0.11.05** | 2026-09-01 | 🐛 **多項修復（用戶實測 — 部署/剷除/網頁卡頓/reconnect 循環）+ 機制意義確認** — ①**FIX dropdown lag（commit 3682d18）**：`/api/activity` 返回全部 10117 條活動記錄 → 前端 render 10117 個 `<tr>` → Chrome 主 thread 阻塞 → dropdown 開唔到 — 修復：前端 render 200 條 + server limit 500（total 欄位）— 實測 render 200 + API 快 20 倍 ②**FIX 部署第二隻冇反應（commit 3da0ea4）**：`doDeployEA` await `_doDeployEA_inner`（入面 await waitDeployDone 最長 5 分鐘）→ `_deploying` 鎖成 5 分鐘 → 第二隻被擋 — 修復：唔 await 成個流程（只鎖提交 5 秒）③**allow_replace 機制（commit c86f99c）**：用戶想用 EA 取代另一隻（代替 dialog）但 auto_attach 硬性撳「否」→ 部署失敗 — 修復：deploy API + deploy_cmd 加 `allow_replace` 參數（用戶確認取代時傳 true → 代替 dialog 撳「是」）④**FIX 部署撞代替 dialog（commit 358be8c）**：開 chart 驗證只 check「有冇 `<SYM>` chart」→ 部署落已有 EA 嘅 symbol → 舊 chart 誤判成功 → attach 落舊 → 代替 dialog — 修復：開 chart 前後數 chart 數量（數量冇增加 = 開 chart 失敗 → 唔 attach）⑤**FIX 新部署 EA 被誤關（commit 061c8c3）**：`_clean_blank_charts` 保留數用「心跳 EA」→ 新部署 EA（OnInit 延遲 — 心跳未寫）誤關 — 修復：讀 MT5 log 最新 loaded EA（5 分鐘內）→ 心跳未寫都保留 ⑥**FIX 剷除誤剷其他 EA（commit 176205c）**：剷 Momentum 誤剷 Breakout（兩個都掛 USDCHF）— `_candidates` 淨按 symbol match → 逐個剷 → 先剷錯 — 修復：用 .chr 檔精準判斷「邊個 chart 掛緊 target EA」→ 淨剷嗰個 ⑦**FIX 剷除後警告視窗未關（commit c868d30）**：刪 show flag 只刪自己目錄 → 開發目錄（alert_worker 讀）殘留 → 警告視窗未關 — 修復：同步刪開發目錄 ⑧**FIX 冇操作都開 MT5 + refresh 導航頁（commit babdd7f + f9d4353 + da8eb17 — 真正 root cause）**：root cause chain 分兩層 — **(a) refresh 導航頁**：agent 每 60 秒 reconnect → server auto-sent EA config（debounce 60s）→ 心跳注入 touch .mq5 → watcher 偵測變化 → refresh Navigator（修復：server debounce 60→300s + agent 心跳已注入唔 touch .mq5 + watcher cooldown 60→300s — commit babdd7f）；**(b) 一刪 MT5 就自動開（真正 root cause）**：網頁每 60 秒 poll `/api/ea-config` → `_market_closed_for_symbol` → `mt5.initialize()` — **MetaTrader5 Python API 自動啟動 terminal64**（未開都開！）+ agent `get_mt5_status()` 同樣問題（修復：server `_market_closed_for_symbol` 先檢查 terminal64 有冇開 → 未開返回 None 唔 initialize — commit da8eb17；agent 加 `_mt5_process_running` helper — 所有 initialize 前檢查 — commit f9d4353）— **實測：關 MT5 → 等 90 秒（網頁 poll 照行）→ MT5 冇自動開 ✅** ⑨**根治 agent reconnect（commit cb0b836）**：server SocketIO 加 ping_interval=25/ping_timeout=60（threading mode 長連接唔穩定 → 60 秒斷）+ agent reconnect backoff（30 秒內唔好試多過一次）⑩**機制意義確認**：心跳注入（驗證 EA 真運行）+ 熱鍵預載（attach EA）+ refresh Navigator（剷除/新增後見到）都係有意義 — 但執行有問題（reconnect 循環 + 無謂動作 — 已修）⑪**統一開 chart 方法（commit e3663eb — user要求：唔好用 Ctrl+N / Ctrl+1-9 / double-click）**：熱鍵測試開 chart Ctrl+N → 改 Alt+F 方法（同部署一致）+ `_exec_open_chart_script`（Ctrl+4 OpenChart_Helper）標記死 code（冇被 call）+ `_safe_double_click` 確認冇實際 call site（死 code）— 全部部署/測試開 chart 統一用 Alt+F + Enter Enter（穩定方法 — user確認）⑫**.chr 機制確立（user實測 — commit c346f4d）**：**機制**：MT5 開住時開新 chart → .chr 檔即刻寫入（Euro folder 即刻見）；MT5 開住時注入 EA → .chr 延遲寫（關閉先寫）；**刪 .chr 唔會即刻停 EA**（只係防 restore — 要 restart MT5 先冇 chart → EA 自然停）；**正確刪 .chr 流程**：一定要關閉 MT5 → 刪 .chr → 重新開 MT5 先成功（MT5 開住刪 → 關閉時 restore 返）— **應用**：①開 chart 驗證加 .chr 數目 check（開 chart 後 .chr 數量增加 = 成功 — 雙重驗證 EnumWindows + .chr）②剷除 CLI `--remove` 改用 `remove_ea_via_chr`（關 MT5 → 刪目標 EA .chr → 開 MT5 — chart 唔 restore — EA 自然停 — 取代 Alt+W 方法 — 解決誤剷其他 EA）— 前提：agent 已修「MT5 未開唔自動開」→ 關 MT5 後唔會即刻重開覆寫 .chr（race 解決）⑬**FIX 剷除開窗口 dialog + 冇刪正確圖表（commit efcb35c — user實測）**：`remove_ea_via_chr` 冇 .chr 檔 → fallback 窗口方法（Alt+W — 開窗口 dialog — user唔想要）+ 誤判「未 running」（心跳 fresh 但 log 話 stopped）→ 冇刪除正確圖表 — 修復：①等 MT5 完全關（4s → 10s + process 確認 — .chr 先 save 完）②掃描全部 profile .chr（Ichimoku 可能喺非 Euro profile — 之前只掃「最近修改」漏咗）③冇 .chr → 唔 fallback 窗口（直接 fail — user要求）④期間 SyntaxError（split 單 backslash + regex 拆行）用 bytes 層面修正 ⑭**FIX 剷除後 MT5 照開 chart（commit b237e30 — user實測：刪 .chr 後 MT5 開機照開返同數目空白 chart）**：root cause — **order.wnd**（MQL5/Profiles/Charts/<profile>/order.wnd — UTF-16 — 記錄 MT5 開機要開邊啲 chart 嘅順序）→ 淨刪 .chr 唔夠（MT5 見 order.wnd 有 chart05 → 重新生成空白 chart05.chr）— 修復：`remove_ea_via_chr` 刪 .chr 後**同步刪 order.wnd 入面對應嘅 chart 項目**（UTF-16 讀寫 — 移除目標 chart 行）→ MT5 開機唔會再開嗰個 chart（成個 chart 移除）— 測試：order.wnd 讀寫 OK（`chart06\r\nchart02\r\n...` 結構）⑮**FIX 剷除後心跳殘留誤判（commit 446af83）**：`remove_ea_via_chr` 刪 .chr 後冇刪心跳檔 → watcher/steps 誤判「EA 仲運行」→ 話 failed（但 EA 已停 — MQL5/Logs「已停止」）— 修復：成功刪 .chr 後一齊刪心跳檔（state_<EA>.json + hb_<EA>.txt — Terminal Common + MQL5/Files）⑯**FIX 剷除「冇 .chr 檔」真正 root cause（commit 7f1a1e0 — user實測：人手部署 EA 落 chart → .chr 即刻有記錄 — .chr 可靠 — 係 code 讀唔到）**：`remove_ea_via_chr` 開頭**冇 `import re`** → `re.search` NameError → try/except 食咗 → 每個 .chr 讀 fail → 話「冇 .chr 檔」→ 剷除 fail（EA 停咗但 chart 冇移除）— debug 揭曉：`name 're' is not defined` — 修復：①`import re` ②等 .chr 檔出現（最多 30 秒 — MT5 關閉 save 大檔慢 — 大 .chr 7MB）③regex `[^\\r\\n]` → `[^<]`（可靠 match EA 名 — `path=Experts\\<EA>.ex5` double backslash）— **實測 PASS**：搵到 chart02（Scalping_M1）→ 刪 .chr → 刪 order.wnd → 刪心跳 → 開 MT5 → **USDJPY chart 成個冇咗（剩 2 個 — EURUSD + UK100）** — 剷除完整成功 |
| **v0.11.04** | 2026-08-31 | 🐛 **FIX #148（Server 靜默 crash — eventlet monkey_patch）+ FIX #154（剷除漏清 — 舊殘留）— 兩項實測 PASS** — ①**#148 root cause**：`RENDER=1` 用 eventlet async_mode **但冇 `monkey_patch()`** → eventlet hub + 原生 thread（彈返監察 threading.Thread）併發 → 底層 C 擴展崩潰（`MSVCP140.dll 0xc0000005` — install-local 期間）— 修復：檔頭加 `eventlet.monkey_patch()`（RENDER 環境 — 必須喺 import socketio 之前 — commit a6209c3）— 實測：server 重啟正常 + install-local（Bollinger_Band 配對）完成冇 crash + **60 個併發請求全部成功**（ok=60 fail=0 — 之前 crash 場景）+ 剷除 Agent 流程冇 crash ②**#154 root cause**：on_shutdown 逐個刪資料夾時冇清「指定舊殘留」（paint_uimap*/Tradotcom-Agent-Setup*/ip-records* — 5 月舊檔殘留）— 修復：on_shutdown 加舊殘留清理（`paint_uimap*`/`Tradotcom-Agent-Setup*.pyw`/`ip-records*`/`*.log`）— 實測：網站撳「剷除 Agent」→ **28 個舊殘留全部清走**（→ 0）+ agent process 全停 + server 端 offline ✅ |
| **v0.11.03** | 2026-08-31 | 🐛 **FIX #157：剷除誤傷其他 EA（重複 pause_cmd + fallback 亂關 chart — Multi_TimeFrame 案例實錘）** + ✅ **混合壓力測試一條龍（stress_combine04 — ① 安裝 → ② 添加 ×3 → ③ 部署 ×3 → ④ 驗證 → ⑤ 剷除 ×3 全 PASS + 23 分鐘錄影）** — ①**#157 root cause（watcher log 鐵證）**：剷除 MACD_Cross 有 2 個 pause_cmd（emit+poll 雙保險）→ 第一個 remove GBPUSD（成功）→ 第二個 remove 時 `_target_sym` 讀唔到（GBPUSD 已關）→ **fallback 揀全部 chart** → 揀 chart [0]（USDJPY）→ Ctrl+W 關錯 → 誤剷 Multi_TimeFrame（18:40:39 removed）②**FIX 1（commit 96aea63）**：`process_pause_cmd` 加 **60 秒 dedupe**（同一 EA 只處理一次 — 同 deploy #152 dedupe 一致）— 實測：`⏭️ Multi_TimeFrame 60 秒內已pause過 — 跳過重複 cmd` ③**FIX 2**：`remove_ea_from_chart` **唔再 fallback 全部 chart**（搵唔到 target symbol → 直接 fail — 唔亂關防誤剷；只容許 log 完全冇記錄時 fallback）— 實測：剷除 Multi_TimeFrame 後 Volume_Spike 保留（AUDUSD chart 冇被誤關）④**混合壓力測試一條龍（錄影 23 分鐘）**：② 添加 ×3 PASS（MACD_Cross + Multi_TimeFrame + Volume_Spike — 警告視窗 steps 完整 + .ex5 + 配對庫）→ ③ 部署 ×3 PASS（GBPUSD + USDJPY + AUDUSD — 心跳 FRESH + MT5 log loaded + **冇誤關**（FIX6/FIX7 work））→ ④ 驗證 PASS（3 隻 runtime 正確 magic/symbol）→ ⑤ 剷除 ×3 PASS（全部 removed — MT5 chart 清空）— cap 圖：screenshots/step2-add-3-ea-stress04-v2.png + deploy-1-macd-stress04-v2.png + deploy-2-multi-stress04-v2.png + deploy-3-volume-stress04-v2.png + remove-all-3-ea-stress04-v2.png + fix157-remove-no-collateral.png — 錄影：recordings/stress-test-combined-full-2026-08-31-v2.mp4 |
| **v0.11.02** | 2026-08-31 | 🐛 **FIX6/FIX7：`_clean_blank_charts` 誤關有 EA 嘅 chart 根治（用戶實測：部署 Multi_TimeFrame 後 MACD_Cross 被誤關 — 心跳計數估算唔準）** + 🛡️ **watcher fingerprint check（用戶實測：安裝完未話開始就自動部署 — 舊 deploy_cmd 殘留觸發）** + ✅ **綜合壓力測試（新 account stress_combine04 — ① 安裝 → ② 添加 ×2 → ③ 部署 ×2 → ⑤ 剷除 ×1 全 PASS）** — ①**FIX6（commit 6c0b8aa）**：`_clean_blank_charts` 改讀 .chr 檔（`path=Experts\<EA>.ex5` — UTF-16 text 有齊 symbol/EA）→ 精準知道「邊個 chart 掛邊個 EA」→ 唔再靠「心跳 EA 數量」估（Volume_Spike + MACD_Cross 都掛 GBPUSD → 心跳計數得 1 → 誤關 MACD_Cross）— 實測：部署 Multi_TimeFrame 後 MACD_Cross 保留（`關閉 0 個`）②**FIX7**：.chr 檔未 sync（MT5 開住啱啱部署完唔 save .chr）→ 合併心跳 EA 最新 loaded symbol（都要保留 — 唔會漏）③**watcher fingerprint check**：`process_deploy` 加 fingerprint 檢查（deploy_cmd 嘅 `fingerprint.account` ≠ 當前 agent_config account → 跳過 + 刪除 — 防舊 account 殘留 deploy_cmd 觸發自動部署「未話開始就開始」）— 實測：舊 account（stress_combine02）cmd → `⛔ 跳過 + 刪除` ✅ ④**綜合壓力測試（stress_combine04）**：① 安裝 4 步 PASS（註冊 stress_combine04 Agent 7f7d669e → 剷除 stress_combine02 本機 agent → 下載 pyw → 安裝精靈 → agent 連線 Registered + 平台服務自動開 + 指紋完整 `account:stress_combine04|agent:7f7d669e`）→ ② 添加 ×2 PASS（MACD_Cross + Multi_TimeFrame — 警告視窗 steps 完整 4 步 + .ex5 生成 + 配對庫顯示）→ ③ 部署 ×2 PASS（MACD_Cross→GBPUSD + Multi_TimeFrame→USDJPY — 心跳 FRESH + MT5 log loaded + 網頁「心跳運行」+ `_clean_blank_charts` 冇誤關）→ ⑤ 剷除 ×1 PASS（MACD_Cross — Alt+W + double check — MT5 log removed ×2 + 心跳停 + 網頁移除 + Multi_TimeFrame 保留）— cap 圖：screenshots/step2-add-macd-stress04.png + step2-add-multi-tf-stress04.png + deploy-both-success-stress04.png + remove-macd-success-stress04.png |
| **v0.11.01** | 2026-08-31 | 🌐 **防死字修正（方案 C）+ 📸 cap 圖規則確立** — ①**防死字**（用戶要求：Python 程式全英文 + emoji 改 ASCII — 見 log 有死字方塊）：9 個 Python 檔案開頭加 sys.stdout.reconfigure(encoding='utf-8')（中文輸出唔再變方塊 — 實測正常）+ 1060 個 emoji/特殊符號 → ASCII（✅→[OK]、❌→[FAIL]、⚠️→[WARN]、🔄→[RETRY] 等 — 10 個檔案）+ 核心 log 片語翻譯英文（deploy/attach/error/success 等 — agent.py/auto_attach.py/deploy_watcher.py/server/app.py）+ 細檔案全翻譯（control_guard/refresh_navigator/deploy_notify/alert_worker/auto_trade_detector）；網頁 template（HTML）保留中文（用戶設定唔郁）— commit 9f4e12a ②**📸 cap 圖規則**（用戶要求：每次做完嘢 cap 圖 send 用戶 — 真實工作畫面成功/失敗睇得出 — 唔係 script 假圖）：Memory + Skill（cap-screenshot-rule.md）雙重記錄；圖 save screenshots/ + 全英文 + 儲存日後用 ③**截圖整理**：警告視窗系列搬去 screenshots/ + 改名清晰（ai-warning-fingerprint-account-hongkpng856.png 等 — commit 8167469） |
| **v0.11.00** | 2026-08-31 | 🔐 **指紋機制（account 追蹤 — 資訊安全開始）** + 🛠️ **#152 修復（auto_attach 重複/queue 卡死）** + 🔒 **關閉公開註冊** + 🔒 **移除 Quick Dev Access** — ①**指紋機制**（用戶要求：所有 account 建立嘅嘢帶指紋）：agent_config.json 加 account/fingerprint 欄位 + 安裝器 UI 加「Account」欄位 + 警告視窗 title 顯示 AI 遠端控制 - account（cap 圖證明 — screenshots/ai-warning-fingerprint-account-hongkpng856.png）+ deploy_cmd/pause_cmd 加 fingerprint（account/agent_id/created_by — 實測 deploy_cmd 有 fingerprint）+ agent.py 啟動 log [FINGERPRINT] Agent 屬於 account + auto_attach --account 參數 + agent.lock 加 account ②**#152 修復**：deploy_cmd 讀完即刻刪（處理前）→ auto_attach running 時 cmd 已冇 → return → 卡死（Mean_Reversion 案例）— 改為 running 檢查喺刪 cmd 之前（running → 保留 cmd 等下次 poll）+ dedupe（同一 EA 60 秒內只處理一次 — emit+poll 雙保險雙 cmd 唔再雙重部署）— 實測：direct call process_deploy 保留 cmd ✅ ③**🔒 關閉公開註冊**：/register 需要邀請碼（TRADOTCOM_INVITE_CODE env / 預設 tradotcom-internal-2026）— 冇邀請碼 403；登入頁隱藏註冊 link ④**🔒 移除 Quick Dev Access**：devLogin 硬編碼 dev 密碼喺前端（安全漏洞）— 刪除按鈕 + 函數 ⑤**Cloudflare 530 修復**：cloudflared tunnel 死（startup bat 用錯舊 tunnel mt5cloud→esgov.org）— 改用正確 9ce5c130→tradotcom.com；重開 tunnel → 200 ✅ ⑥**account 重置**：全部舊 account（dev/hongkpng856/stress09/test_final）剷除 + 開新 account hongkpng856（TEST01 agent） |
| **v0.10.99** | 2026-08-31 | 🎯 **2號壓力測試 5/5 PASS（random 數量 1-3 + random EA + random symbol + 雙層認證）** + 🔒 **關閉公開註冊（內部限定）** — ①**5 次測試全 PASS**：Test 1（Swing_Trader→EURUSD）、Test 2（Swing_Trader→USDJPY + RSI_Over→AUDUSD）、Test 3（Fibonacci→AUDUSD）、Test 4（Stochastic→EURUSD + Martingale→GBPUSD + Momentum→USDJPY）、Test 5（Martingale→AUDUSD + Volume_Spike→EURUSD + Mean_Reversion→USDJPY）— 每次操作**雙層認證**（網頁 config/runtime + 電腦 MT5 log/心跳/chart + **電腦版警告視窗有彈** + steps 演進）— 添加/部署/剷除全 PASS；⚠️ 記錄 bugs：#150（開 chart 重試開多 chart）+ #152（auto_attach queue 唔自動處理 — 部署卡住要重試）+ #148 server crash 再現 + 過午夜 log 檔切換 ②**🔒 關閉公開註冊**（用戶要求：網上唔好俾人註冊 — 淨係內部帳戶做到嘢）— `/register` 頁 + `/api/register` 改內部限定（需要邀請碼/管理員開通 or 直接禁用）— 待實施 |
| **v0.10.98** | 2026-08-29 | 🐛 **alert_worker 死（警告視窗冇彈 — 2號壓力測試雙層認證發現）** — alert_worker `main()` while loop `except tk.TclError: break` → **WM_CLOSE 關視窗（確定掣）→ root.update() 拋 TclError → break → 成個 process 死** → :5004 冇 LISTEN → 之後所有警告視窗都唔彈（show flag 有寫但冇人讀）；測試實錘：添加/剷除警告視窗有彈，但部署期間 alert_worker 死 → 部署警告視窗冇彈；修復：TclError 改**重建視窗**（記錄 + destroy + `tk.Tk()` 重建 + `shown=False` — process 保持生存 — 下次 flag 再彈）— **實測：WM_CLOSE 後 alert_worker 仲生（:5004 LISTEN）+ 窗口關閉 ✅**；同步開發目錄 + TradotcomAgent 兩邊 |
| **v0.10.97** | 2026-08-29 | 🎯 **2號壓力測試 5/5 PASS（每輪 2 EA 部署+剷除 — 模擬真人點擊）** + 🐛 **Server 靜默 crash（install-local 期間）** — ①**2號測試 5 輪全 PASS**：10 次部署 + 10 次剷除（ATR_Stop→EURUSD/MACD_Cross→GBPUSD → Bollinger→USDJPY/EMA→AUDUSD → ADX→XAUUSD/Support→GBPUSD → Breakout→USDJPY/Correlation→EURUSD → ATR→XAUUSD/MACD→USDJPY）— 全部 MT5 log loaded/removed + 心跳 fresh + chart 0；連環部署鐵律（逐隻等完成先下一隻）work ②**install-local 雙寫修復**：複製完成更新 steps + compile 失敗段改用 `_write_ai_flags` 雙寫（之前只寫開發目錄 → alert_worker 讀 TradotcomAgent steps 卡「開始配對 doing」→ 警告視窗卡住「操作中」— 修復後兩邊 steps 全部 done + 視窗正常顯示「已完成」）③**🐛 Server 靜默 crash**：壓力測試期間 server 喺 install-local 死 3 次（06:16/06:27/06:57 — log 停喺「複製監察」後冇 traceback）；Windows Event Log 顯示 `pythonw.exe MSVCP140.dll 0xc0000005`（底層記憶體違規 — eventlet/threading 併發問題：install-local 長 request + agent 每 5 秒 poll + SocketIO → 撞）— **影響：用戶添加/部署突然冇反應（server 死咗）；每次要手動重啟 server** — ⚠️ 待根治（加 watchdog 自動重啟 or 查 eventlet 資源洩漏） |
| **v0.10.96** | 2026-08-29 | 🐛 **配對庫殘留 EA（電腦冇 EA 但配對庫有 — 用戶實測「重新整理咗都仲係度」— 實測 PASS）** — `api_ea_config` `_agent_eas` 計算 `_hb_e = heartbeats.keys()` **冇 alive 過濾**（只過濾咗 `_log_e`）→ 死 agent 嘅舊 snapshot（殘留心跳檔上報 — `status: 'stale'`）包含已剷除 EA → 前端 `agentEasCache` 有殘留 → 配對庫顯示「已加入」殘留 EA；修復：`_hb_e` 改用 `_hb_alive`（心跳 `status == 'alive'` 先計 — 同 `_log_e` 一致）— **實測：dev account agent_eas `['Bollinger_Band','EMA_Cross','Support_Resist']` → `[]`（全部 stale 剔除）+ stress09 都 `[]`；新 account 唔會再有（只有 alive 心跳先顯示）** |
| **v0.10.95** | 2026-08-29 | 🐛 **「重新整理」電腦版警告視窗冇彈 + EA 倉庫/配對庫冇更新（人手壓力測試發現 — 實測 PASS）** — ①**電腦版警告視窗冇彈**：「重新整理」流程（`api_control_steps` POST + `api_ea_library_refresh`）寫 `.ai_control.show/.steps` **只寫開發目錄**（`<repo>/agent`）→ alert_worker（讀 `%LOCALAPPDATA%\TradotcomAgent`）讀唔到 → 電腦版唔彈（a1cffdd 修咗 install-local/部署但漏咗 refresh 路）；修復：5 個位置改用 `_write_ai_flags` 雙寫（開發目錄 + TradotcomAgent）+ 雙刪 show flag ②**EA 倉庫/配對庫冇更新**：壓力測試剷除後**殘留 5 個 `state_*.json` 心跳檔 + 10 個 `web_add/web_delete` flag + hotkeys.ini 殘留 `Support_Resist=Ctrl+1`** → agent `build_files_snapshot` 上報（heartbeats/hotkeys）→ server `_agent_eas` 包含已剷除 EA → 前端配對庫顯示「已加入」殘留 EA（心跳暫停）；修復：清心跳檔 + flags + hotkeys.ini → `agent_eas` 由 5 → 0 — **實測：`/api/control-steps` POST 後開發目錄 + TradotcomAgent 兩個位置都有 show flag（之前 TradotcomAgent 係冇）+ agent_eas 空** |
| **v0.10.94** | 2026-08-29 | 🐛 **剷除假失敗（窗口 dialog 誤判 — EMA_Cross 案例）** — auto_attach `remove_ea_from_chart` 剷除時：①`_dlgs()` 掃全桌面所有 `#32770` dialog → 其他 dialog（殘留/其他 app）title 含「窗口」→ 誤判「窗口 dialog 未關」→ 剷除中止（假失敗 — EA 其實已移除）；修復：`_dlgs()` 精準匹配（class #32770 + title 以「窗口」開頭 或 "Windows"）②二次 Enter 都關唔到 dialog → 盲報失敗；修復：二次 fail 前加**心跳停 / MT5 log removed 最後確認**（EA 真移除 = 唔當失敗 — 繼續關 chart）— **實測 PASS**：部署 Bollinger_Band→EURUSD ✅ + 剷除 ✅（MT5 log removed + 心跳停 — 冇再卡「窗口 dialog 未關」）、部署 Support_Resist→GBPUSD ✅ + 剷除 ✅（兩次完整 cycle） |
| **v0.10.93** | 2026-08-28 | 🏷️ **品牌定案 + Landing 全面更新（用戶要求：中文名交易點 + 點字金色 + icon 用返自己圖 + slogan 交易點一點 + 登入/註冊交叉掣 + 主頁導航 — 實測 PASS）** — ①**中文名「交易點」**（交易嘅地方/如何交易 — 唔再叫交易掂）+「點」字用金色（nav + footer + i18n — b959c5b）②**Slogan「交易點一點」**（解讀1：交易點，一點 / 解讀2：交易點一點 — 35d3ecb）③**Icon 用返用戶原圖**：金色方塊 + **白色 T** + 黑陰影 — flood fill 去背景（角落連通白色先刪 — 金色入面嘅白色 T 保留 — 之前 threshold 去背誤刪白色 T — 35d3ecb）+ 全平台統一（landing nav/hero/footer + login + register + dashboard sidebar — 5201106）④**登入/註冊加交叉掣**（✕ 右上角 — 返回主頁 — 還原 icon-font logo 好似之前版本 — a0f3e33）⑤**主頁導航**：login/register logo 可撳返 `/` + 新增 `/home` route（已登入都顯示 landing — 唔 redirect）+ dashboard sidebar 加「首頁」link（1f23697）⑥**Landing 正式書面文字 + 雙語**（繁中 + English toggle — 特點×8 真實 + 比較 5 欄 MetaApi/VPS/MQL5 Signal/MT5 VPS — 7f5d858）⑦**Logo 互動**（左上靜止 + Hero 大標題 icon 點擊動畫 + 音效 + 操作 GIF 錄製 — 6983498/0c4d33e）— **⚠️ Dashboard「碌落最底彈返主頁」用戶實錘**（測試未能重現 — URL 冇變 — 懷疑 sidebar「首頁」link 誤觸發 — 待觀察） |
| **v0.10.92** | 2026-08-28 | 🌐 **Landing page + tradotcom.com 正式域（用戶要求：一個 domain 兩用 — 介紹網站 + 平台 — 實測 PASS）** — ①**Landing page（UI/UX Pro Max skill）**：橙黑 Binance 風格（#f0b90b/#0b0e11/DM Sans — 提取 reference demo real tokens — 同 UI 定案一致）+ 結構（NAV/Hero/真實 dashboard 截圖/特點×6/比較表格/CTA/Footer）+ 內容配合 Tradotcom 產品（唔係 crypto 交易所 — 多用戶 MT5 控制 + EA 自動化 + P&L 統計 + 中文支援）+ 比較表格（Tradotcom vs VPS 托管/跟單平台/遠端桌面 — 9 項我哋有佢哋冇）②**tradotcom.com 正式域（Cloudflare）**：DNS CNAME record（`@` + `www` → `9ce5c130-ff5d-4ff0-9460-95ab2aab50dd.cfargotunnel.com` — 用戶 dashboard 手動加）+ tunnel config.yml 加 hostname（tradotcom.com + www + mt5cloud.esgov.org 全部 → localhost:5001）+ tunnel 重啟（4 connections — hkg09/11/10/01 QUIC）+ 驗證 https://tradotcom.com 200（load 到平台）③**融入 production**（ea4e647）：`/` route 未登入 → render `landing.html`（新介紹網站）+ 已登入 → dashboard + 真實截圖 → `/static/dashboard-shot.png` — 實測：`/` 200 + 含 landing 標題/特點/比較/註冊掣 ✅ |
...
| 145 | **🔥 電腦版警告視窗冇彈（「重新整理」流程 — 人手壓力測試發現）** | 「重新整理」流程（`api_control_steps` POST + `api_ea_library_refresh`）寫 `.ai_control.show/.steps` **只寫開發目錄**（`<repo>/agent`）→ alert_worker（讀 `%LOCALAPPDATA%\TradotcomAgent`）讀唔到 → 電腦版唔彈（a1cffdd 修咗 install-local/部署但**漏咗 refresh 路**）| v0.10.95 5 個位置改用 `_write_ai_flags` 雙寫（開發目錄 + TradotcomAgent）+ 雙刪 show flag（成功/失敗兩個分支）。實測：`/api/control-steps` POST 後兩個位置都有 show flag（之前 TradotcomAgent 係冇）| 08-29 |
| 146 | **🔥 EA 倉庫/配對庫冇更新（殘留心跳檔 + flags + hotkeys 污染 agent_eas — 人手壓力測試發現）** | 壓力測試剷除後**殘留 5 個 `state_*.json` 心跳檔 + 10 個 `web_add/web_delete` flag + hotkeys.ini 殘留 `Support_Resist=Ctrl+1`** → agent `build_files_snapshot` 上報（heartbeats/hotkeys）→ server `_agent_eas` 包含已剷除 EA → 前端配對庫顯示「已加入」殘留 EA（心跳暫停 — 用戶以為「冇更新」）| v0.10.95 清心跳檔 + flags + hotkeys.ini → `agent_eas` 由 5 → 0（前端配對庫正確顯示空）。⚠️ 教訓：剷除後要清心跳檔 + flags + 熱鍵（唔係淨刪 .ex5）| 08-29 |
| 147 | **🔥 配對庫殘留 EA（電腦冇 EA 但配對庫有 — 用戶實測「重新整理咗都仲係度」）** | `api_ea_config` `_agent_eas` 計算 `_hb_e = heartbeats.keys()` **冇 alive 過濾**（只過濾咗 `_log_e`）→ 死 agent 嘅舊 snapshot（殘留心跳檔上報 — `status: 'stale'`）包含已剷除 EA → 前端 `agentEasCache` 有殘留 → 配對庫顯示「已加入」殘留 EA（即使本機冇 EA + config 空）| v0.10.96 `_hb_e` 改用 `_hb_alive`（心跳 `status == 'alive'` 先計 — 同 `_log_e` 一致）。實測：dev account agent_eas `['Bollinger_Band','EMA_Cross','Support_Resist']` → `[]`（全部 stale 剔除）+ stress09 都 `[]`。⚠️ 教訓：所有「agent 上報」來源（heartbeats/log_last/hotkeys）都要 alive 過濾 — 死 agent 舊 snapshot 唔可以計入 | 08-29 |
| 148 | **🔥 Server 靜默 crash（install-local 期間 — 2號壓力測試發現死 3 次）** | server 喺 install-local（添加 EA）期間**靜默死**（log 停喺「複製監察」後冇 traceback — port 5001 冇 LISTEN）；Windows Event Log：`pythonw.exe MSVCP140.dll 0xc0000005`（底層記憶體存取違規）；模式：server 行緊（agent 每 5 秒 poll `/api/agent-poll-deploy` + 前端 poll `/api/ea-config` + SocketIO）+ install-local 長 request（等 compile 45 秒 + config 寫入）→ **eventlet/threading 併發撞** → crash。影響：用戶添加/部署突然冇反應（server 死咗）+ 前端「撳部署冇反應/警告視窗彈走」 | ⚠️ 待根治（v0.10.97 記錄）：①server 加 watchdog 自動重啟（偵測 port 死 → 自動開返）②查 eventlet 資源洩漏（長 request 併發）③或 install-local 改非同步（唔等 compile — 即刻 return + watcher 補 config） | 08-29 |
| 149 | **🔥 alert_worker 死（警告視窗冇彈 — 2號壓力測試雙層認證發現死 2 次）** | alert_worker `main()` while loop：`except tk.TclError: break` → **WM_CLOSE 關視窗（確定掣）→ root.update() 拋 TclError → break → 成個 process 死** → :5004 冇 LISTEN → 之後所有警告視窗都唔彈（show flag 有寫但冇人讀 → 用戶「警告視窗冇彈」）；測試實錘：添加/剷除警告視窗有彈（alert_worker 生），但部署期間 alert_worker 死咗 → 部署警告視窗冇彈 | v0.10.98 `except tk.TclError: break` 改為**重建視窗**（記錄 + destroy + `tk.Tk()` 重建 + `shown=False` — process 保持生存 — 下次 flag 再彈）。實測：WM_CLOSE 後 alert_worker 仲生（:5004 LISTEN）+ 窗口關閉 ✅ | 08-29 |
| 150 | **🔥 auto_attach 開 chart 重試開多 chart（2號測試發現 — Round 4/5 開咗 5-6 個重複 EURUSD chart）** | auto_attach 開 chart 失敗 → **重試邏輯每次開一個新 chart**（唔係重用失敗嗰個）→ 累積好多重複 chart（Round 4 開 5 個 EURUSD、Round 5 開 6 個）→ 之後**剷除時 ListView 唔齊**（有啲 chart 隱藏/重複 → 揀唔啱 chart）→ EMA_Cross 剷除失敗（心跳仲 fresh — 要手動 WM_CLOSE 全部 chart 先清到）| ⚠️ 待修（v0.10.99 記錄）：①開 chart 重試前**先檢查有冇同 symbol chart 已存在**（重用 — 唔開新）②剷除時用 EnumChildWindows 搵全部 chart（唔靠 ListView）③部署後清空多餘 chart | 08-29 |
| 151 | **🔥 Demo_Data.mq5 compile 失敗（5 errors — 唔應該喺官方庫）** | Demo_Data.mq5 源碼有 5 個 error：`error 256: undeclared identifier '__demo_process'` + `error 157: ')' expected` + `error 199: wrong parameters count`（iATR/iRSI/iMA 函數參數錯）→ compile 失敗 → 冇 .ex5 → 添加咗但部署唔到（random 壓力測試揀中佢先發現）| ⚠️ 待處理（v0.10.99 記錄）：①修 Demo_Data.mq5 code（補 `__demo_process` 定義 + 修正 iATR/iRSI/iMA 參數）②或者從官方庫移除（唔應該俾用戶添加）③random 測試排除 compile 失敗 EA | 08-29 |
| 152 | **🔥 auto_attach queue 唔自動處理（部署卡住 — 2號測試 random 發現）** | watcher 處理 deploy_cmd 時偵測到「auto_attach.py already running」→ **queue 咗下一個部署**（deploy_cmd 已刪）→ 但 auto_attach 完成當前任務後**冇自動攞 queue 下一個** → 下一個 EA 部署卡死（Mean_Reversion 案例 — 等 auto_attach 完成後要手動再 POST deploy 先成功）| ✅ 已修（v0.11.00）：running 檢查**喺刪 deploy_cmd 之前**（running → 保留 cmd 等下次 poll — 唔刪）+ **dedupe**（同一 EA 60 秒內只處理一次 — emit+poll 雙 cmd 唔再雙重部署）— 實測：direct call process_deploy 保留 cmd ✅ | 08-29 |
| 153 | **🔥 Python log 死字方塊（console 中文/emoji 變方塊 — 用戶見識）** | Python console 輸出中文 + emoji（✅❌⚠️🔄 等）喺 GBK/cp936 console 顯示變死字方塊 — 影響 log 可讀性 + 用戶睇唔明 | ✅ 已修（v0.11.01 方案 C）：①9 個 Python 檔案加 `sys.stdout.reconfigure(encoding='utf-8')`（中文輸出 UTF-8 — 唔再變方塊）②1060 個 emoji → ASCII（[OK]/[FAIL]/[WARN]/[RETRY]）③核心 log 片語 → 英文；網頁 template 保留中文（用戶設定）— 實測輸出正常 | 08-31 |
...
**Git HEAD**: `9f4e12a`（master）— v0.11.01（防死字修正 + cap 圖規則）；TODO：數據注入選擇功能未實行（見 TODO 段）
