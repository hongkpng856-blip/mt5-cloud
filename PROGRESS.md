...
| # | 描述 | 狀態 | 影響 |
|---|---|---|---|
| 173 | **企業級自動更新機制（41628e4 — 09-05）** — 之前 agent code 每部機人手 sync（A 機我手動 cp — B 機仲行緊舊 code — 冇 state_.json 掃描/心跳修復 → B 機配對庫空白）→ **加自動更新**：①`AGENT_VERSION`（agent.py 開頭）②server `/api/agent-version`（讀自己 agent.py 版本 — server 永遠最新）③agent `_check_agent_update`（啟動 15 秒後 check — 有新版 → **tkinter 彈視窗**「Tradotcom Agent 有新版本 X → 而家更新？」→ 用戶確認 → 下載 /api/agent-py + 平台服務檔 → 驗證（syntax + AGENT_VERSION）→ 覆蓋 → 自動重啟）— **注意：B 機而家行緊舊版（冇更新 code）→ 要手動更新一次（之後先會自動）** — **B 機 = claw（username — 唔係 hongk！）agent 路徑 = `C:\Users\claw\AppData\Local\TradotcomAgent`**（之前一直以為係 hongk — 錯）| 🟡 進行中 | A 機已同步 41628e4（28324 行緊新版）— B 機待手動更新一次（VPS restart 後）|
