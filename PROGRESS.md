...
| 版本 | 日期 | 內容 |
|------|------|------|
| **v0.9.9** | 2026-08-07 | 🎯 開圖表突破（用戶發現：**Alt+F → Enter → Enter** 開新圖表 — 取代 Ctrl+N（導航開關 — 唔係開圖表）+ 前端品種選擇 UI（部署彈 modal 揀 EURUSD/USDJPY/GBPUSD...）+ OpenChart（MQL5 ChartOpen — 支援指定 symbol — EA 版 OnInit+ExpertRemove）+ 熱鍵失效 root cause（冇圖表 — 熱鍵需要 active 圖表）+ 多圖表多 EA 同時運行驗證（Swing_Trader/TestRunner 心跳 ✅ 圖表 4）|
