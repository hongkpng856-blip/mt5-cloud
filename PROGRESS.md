...
| # | 描述 | 狀態 | 影響 |
|---|---|---|---|
| 172 | **網頁版警告視窗 steps 同步（已修）+ 清理空白圖表按鈕「清理中」唔復原（未收好）** — ①**網頁版警告視窗 steps 同步（已修 — 09-04）**：agent（A 機）control_steps 上報 server 冇 work（server log：控制 steps 空/冇）→ **root cause：agent 17976 行緊舊 code**（冇/壞 control_steps 上報）→ **重啟 agent（1608 — 最新版）→ 上報正常**（VPS steps 由 agent 上報覆蓋 — Removed 0 — 網頁同步 ✅）②**清理空白圖表按鈕（網頁版）已修 4 個**：a) 網頁版冇警告視窗 — cleanBlankCharts 漏咗 showControlModal（d286448 加返）b) modal 空白 — showControlModal 喺 server steps 寫好前彈 → steps mtime 舊 → 改 API 成功後先彈（75b8037）c) 5 秒 auto reload 打斷 — 改延遲 refresh 由 pollAiControl 控制（d03db74）d) 電腦版卡最尾步驟 — auto_attach `_update_steps` 累積殘留 → 直接覆寫雙寫（7d97902）③**按鈕「清理中」唔復原（未收好 — 09-04 記錄）**：清理完成後按鈕依然寫「清理中」— 已加 allDone 復原（dcd84c5 — 靠 pollAiControl — modal 關咗唔 poll）+ 獨立輪詢復原（f870660 — 每 3 秒 poll steps 全 done 復原 — 最長 60 秒兜底）— **用戶實測仍唔復原 — 待重測/再查（可能 VPS 未 restart 食 code / btn selector 唔啱 / steps 空情況）** | 🟡 部分修 | 網頁版 steps 同步已修（agent 重啟後正常）— 清空白按鈕復原未收好（待重測） |
