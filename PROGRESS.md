...
| 163 | **`verify_heartbeat` 喺 deploy_ea_via_chr 內 timing 問題** — EA 實際掛咗（MT5 log loaded + 心跳 FRESH + OnInit「已啟動」）但 verify_heartbeat 話「未確認」（restart 後心跳寫入慢 — 90s timeout 後先寫）→ watcher/steps 顯示 failed（但實際成功） | 🟡 待優化 | 部署表面 failed（實際成功）— steps 顯示慢 |
...
> 🎉 2026-09-01：#152/#156/#157/#160/#161/#162（dropdown lag/部署第二隻/代替 dialog/剷除誤剷/冇操作開 MT5 真正 root cause/reconnect 循環/剷除「冇 .chr 檔」真正 root cause）已修（見版本表 v0.11.05/v0.11.06）。
> ⏳ 2026-09-01：#163（deploy_ea_via_chr 內 verify_heartbeat timing — EA 實際掛咗但 verify 話未確認）待優化。
