...
| **v0.10.70** | 2026-08-24 | 🔧 **人手模擬壓力測試（網頁撳）×3 + watcher 修復** — ①**watcher 讀舊 output 假成功**：auto_attach 死喺中途 → aa_debug.log 殘留上次 SUCCESS → watcher 誤判成功 → spawn 前清 aa_debug.log ②**watcher 剷除假成功已正確報告**：check auto_attach output（「未能確認移除」→ 寫「❌ 剷除失敗」+ 通知用戶 — 之前照寫「已暫停」）③**舊 watcher 用舊 code**：pythonw 長駐 watcher 唔會 reload — 改 deploy_watcher.py 後要 kill + 重啟先生效 — 測試結果：輪 1 EMA PASS / 輪 2 Grid 部署 PASS（剷除要手動補）/ 輪 3 ADX 假成功 → 修復後真成功 |
...
| 108 | **watcher 讀舊 output 假成功（部署 Breakout 話成功但實際失敗）** | auto_attach 死喺中途（MT5 kill）→ tee 覆寫 aa_debug.log 唔完整 → watcher 讀到上次部署嘅 SUCCESS 殘留 → `_aa_ok` True → 誤判成功 + 寫「部署完成」+ server report ✅ | v0.10.70 watcher spawn auto_attach 前**先清 aa_debug.log**（確保讀到嘅一定係今次 output）。實測：清咗之後 Breakout/ADX 部署結果正確反映 | 08-24 |
...
**Git HEAD**: `3092c4b`（master）— v0.10.70（熱鍵先係主力 + 假成功修復：verify_heartbeat 讀 terminal Logs + 心跳 age check + watcher 清 aa_debug）；TODO：數據注入選擇功能未實行（見 TODO 段）
