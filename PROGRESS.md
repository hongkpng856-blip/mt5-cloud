...
| **v0.10.82** | 2026-08-26 | 🏷️ **品牌改名（用戶：「我哋呢個品牌叫做 Tradotcom 取代 MT5 Cloud + EA 管理改為 Alpha 管理」）** — ①全部 UI 字眼：**MT5 Cloud → Tradotcom**（dashboard header / index / login / register title+h1）②**EA 管理 → Alpha 管理**（配對庫 card title）③EA library .mq5 header comment/copyright（9 個檔）④server/app.py + agent/*.py comments + startup print — 實測：`<title>Tradotcom</title>` + 登入頁 Tradotcom（冇 MT5 Cloud 殘留） |
...
**Git HEAD**: `b5e1c68`（master）— v0.10.82（品牌改名 Tradotcom + Alpha 管理）；TODO：數據注入選擇功能未實行（見 TODO 段）
