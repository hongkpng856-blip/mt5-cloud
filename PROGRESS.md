...
| 版本 | 日期 | 內容 |
|------|------|------|
| **v0.9.26** | 2026-08-12 | 🎯 基礎設施大修（轉 Agent 後全面修復）：**detector 讀所有用戶 config**（唔再 hardcode 'dev' — 切換 Agent 後 dev 0 keys 導致永遠 No EAs）+ **server 讀 auto_trade_status.json**（唔直接 init MT5 — 同 detector 衝突）+ **detector account_info 合併到 payload**（balance/equity 顯示）+ **dashboard JS syntax fix**（sc0 重複宣告 → 全部 JS 死）+ **detector account_info 喺 initialize 後即刻攞**（EA 計算後 disconnect 返回 None — 移前）\| 🎯 警告視窗重製大修（平台核心 — 用戶願景：所有人透過佢知道遙距控制成功/失敗）：**重製**（電腦版 + 網頁版一致 — 「遙距控制」標題 🤖 + 操作名 + 步驟 + 二選一）**視窗抖動根治**（電腦版：minsize/maxsize 鎖死 + 移除每 round geometry（觸發 re-layout）+ 增量渲染（內容一樣 skip）；網頁版：modal 固定高度（min-height 430 + 步驟區 120-160 scroll）+ 預留 6 行（空行填補 — 位置唔跳））**真逐步**（後端每步 0.8 秒 — 剷除 6 步活動記錄式：開始 → 檢查圖表 → 移除 EA → 刪檔案 → 清理設定 → 完成 — 網頁捕到每步「進行中」）**按鈕統一**（確定按鈕 4 處 set → 1 處（steps render 決定 — 二選一唔會兩個同時））**poll 加快**（2 秒 → 0.7 秒 — 捕到每步 — 內容平滑）**remove-local 唔雙重寫**（等 watcher 接手 — 唔會覆蓋 steps）**steps 讀容錯**（JSON 損壞 → 「等待操作開始」唔空白）**_clear_steps 寫「等待操作開始」**（唔係空 [] — 唔會空白彈）**版本標記**（右下角 v0.9.25-dev — 確認載入新版）⚠️ 已知未解決（2026-08-12 — 交俾另一 AI）：網頁版警告視窗「彈嚟彈去」— 用戶實測「冇改變過」。**實據（steps_monitor.log + 影片抽幀 + Vision AI）**：① 後端 steps 完全正常（剷除逐步 6 步每步 1 秒冇反覆）② **影片 f_007 捕捉到「瀏覽器原生 confirm（確定要刪除 Breakout？）」**（deleteEA 嘅 confirm — 已移除）③ f_011 捕捉到「舊任務（編譯完成畫面）殘留」④ f_003/f_004 捕捉到「步驟區空白」（steps 空 → 網頁清空 — 已修唔清空）⑤ PIL 掃描 59 幀發現「內容突變」時刻（f_007/011/031/042/049/056 — 綠/灰像素突變）。**已做全部修復（v0.9.25）**：重製（遙距控制標題）/視窗抖動根治（minsize/maxsize+增量渲染+預留行數）/真逐步（後端每步 0.8 秒 6 步）/按鈕統一（4處→1處 二選一）/poll 加快（2秒→0.7秒）/remove-local 唔雙重寫/讀 steps 容錯/_clear_steps 寫「等待操作開始」/移除 mtime 判斷/空 steps 唔清空/移除 deleteEA 原生 confirm/**原子寫入（17 處 tmp+rename）**。**下一 AI 方向**：如果仲有「彈」→ ① 睇最新影片抽幀（ffmpeg 已裝 — imageio_ffmpeg）② 用 Vision AI（auxiliary vision = opencode-go/mimo-v2.5 — 已設定成功）逐幀睇 ③ 前端 pollAiControl render 徹底重寫（乾淨版 — 淨係跟 steps 顯示 + 二選一）④ 檢查「舊任務殘留」（showControlModal 後 — 新任務 steps 未寫入 — 舊內容顯示 1 秒）⑤ 考慮「內容含 EA 名判斷」（steps 含當前操作 EA 名先顯示 — 唔含 → 等待）⚠️ Vision AI 設定方法：`hermes config set auxiliary.vision.model mimo-v2.5`（provider=opencode-go 已設 — 錯誤「Console Go」= 主模型通道 — auxiliary 用 mimo-v2.5 成功）\|
