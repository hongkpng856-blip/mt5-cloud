...
| **v0.10.71** | 2026-08-24 | 🔥 **人手模擬壓力測試（網頁撳）×5 達 4/5 PASS（熱鍵附加根治）** — ①**熱鍵 load 測試 check EA 名**（之前 `'1.00' in 標題` 就當成功 — 彈錯 EA（Heikin_Ashi 殘留）都 pass → 部署掛錯 EA）→ 改 `ea_name in 標題`（一定要彈返目標 EA）②**熱鍵 load 失敗 → 第二次 restart**（MT5 開機 cache 舊 hotkeys — 第一次 restart 後 load 唔到新 mapping → 再 restart 一次 reload — 實測 Breakout/MACD/Heikin 第二次 restart 後熱鍵 load 到）③**send 熱鍵重試 5 次**（send 前等 5 秒 + 冇彈 Properties 重試 — MT5 熱鍵 load 慢）④**測試 script 等 240 秒**（兩次 restart 要 3-4 分鐘 — 之前等 120s 太早 DELETE）— 結果：輪 1 ATR PASS / 輪 2 Bollinger FAIL（開 chart 偶發失敗）/ 輪 3 Breakout PASS / 輪 4 MACD PASS / 輪 5 Heikin PASS = **4/5 PASS**（熱鍵附加全成功 — 唯一失敗係開 chart 偶發） |
...
| 111 | **測試 script 連環部署太早 DELETE（部署未完就刪 — 假 FAIL）** | 測試 script 部署後等 120 秒就驗證/刪除 — 但熱鍵預載兩次 restart 要 3-4 分鐘 → 部署未完就 DELETE → 混亂假 FAIL | v0.10.71 測試 script 等 240 秒（兩次 restart 完成先驗證） | 08-24 |
...
**Git HEAD**: `291c25e`（master）— v0.10.71（熱鍵附加根治：load 測試 check EA 名 + 第二次 restart reload 熱鍵 + send 重試 5 次 — 人手模擬測試 4/5 PASS）；TODO：數據注入選擇功能未實行（見 TODO 段）
