...
| # | 描述 | 狀態 | 影響 |
|---|---|---|---|
| 171 | **VPS 搬遷 — 配對/剷除卡住 + 各項遠端化修復（方案2 核心工程）** — server（VPS）冇 MT5 — 配對（install-local）原本 server 自己複製/編譯去 VPS（冇 MT5 → 卡）；剷除（remove-local）同樣卡。**已修（一連串 commit）**：①**配對+剷除遠端化（d5d9eb8）**：server 偵測 agent connected → 發 `install_ea_command`/`ea_remove_command`（SocketIO）→ agent 自己機下載+安裝+編譯 / 刪檔案+寫 pause_cmd（watcher remove chart）②**EA庫 download 404（03a0c41）**：EA_LIBRARY_DIR 等相對路徑 → abspath（新 werkzeug safe_join 拒 `..`）③**EA庫 local_has 用 agent snapshot（ba313a6）**：agent `build_files_snapshot` 加 local_eas 掃描（Experts/Scripts .mq5/.ex5）→ server 唔再讀 VPS 檔案 ④**Steps+Symbols 上報（39d08d8）**：agent sync 帶 control_steps（本地 .ai_control.steps）+ symbols（過濾後）→ server 寫自己 steps（網頁同步）+ symbol dropdown ⑤**Symbols 過濾（56125f8）**：12455（全部市場 incl 股票）→ 58（FX/金屬/指數 — 過濾 6位 XXXYYY 尾主要貨幣 + XAU/XAG/XPT/XPD 開頭 + 常見指數）⑥**Agent 斷線循環（1b5ba0f）**：reconnect 全改 polling（VPS eventlet websocket 唔穩定 — 每 ~50 秒斷）⑦**配對卡住（c14794c）**：agent 完成後寫 steps done + 清 show flag → 網頁唔再卡「進行中」⑧**Compile 卡死（6a2205e）**：compile 前 kill 舊 MetaEditor（單實例佔用 → subprocess 卡 120 秒）— **API 測試 PASS**：配對 TestStoch → XPTUSD 部署（MT5 log loaded successfully + 心跳 running）→ 剷除（log removed + 檔案刪）— **verify: server steps 全部 done（網頁同步）** | ✅ 已修 | 配對/剷除/部署遠端化完成（A 機 + VPS 測試 PASS）— 待人手驗收 |
