...
| 152 | **🔥 auto_attach queue 唔自動處理（部署卡住 — 2號測試 random 發現）** | watcher 處理 deploy_cmd 時偵測到「auto_attach.py already running」→ **queue 咗下一個部署**（deploy_cmd 已刪）→ 但 auto_attach 完成當前任務後**冇自動攞 queue 下一個** → 下一個 EA 部署卡死（Mean_Reversion 案例 — 等 auto_attach 完成後要手動再 POST deploy 先成功）| ⚠️ 待修（v0.10.99 記錄）：①watcher queue 機制要 auto_attach 完成後自動觸發下一個 ②或者唔好 queue — 等 auto_attach 完成先寫 deploy_cmd（poll 機制已有）③部署前檢查 auto_attach 狀態（running → 等佢完成先 send） | 08-29 |
...
**Git HEAD**: `ba11540`（master）— v0.10.98（alert_worker TclError 重建修復 + 2號測試雙層認證）；TODO：數據注入選擇功能未實行（見 TODO 段）
