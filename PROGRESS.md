...
| # | 描述 | 狀態 | 影響 |
|---|---|---|---|
| 168 | **VPS 搬遷（進行中）** — server 由本機搬去 VPS（116.206.150.233 — Windows Server 2012 R2 — 1 CPU/2GB）— ✅ 已做：①VPS 裝 Python 3.8.10（3.11 唔支援 Server 2012 — stopped working — 裝 VC++ Runtime 解決）②server code + DB 搬過去（RDP 複製 zip）③pip 裝依賴④server 啟動（PORT=80 — RENDER=1 eventlet — 注意：RENDER=1 冇 eventlet 會 `invalid async mode` error → 裝 eventlet 或者唔設 RENDER）⑤防火牆開 80/5001⑥Cloudflare DNS：CNAME tunnel → A record 指 VPS IP + SSL Flexible⑦agent 測試：**經 CF（https://tradotcom.com）每分鐘斷線**（CF WebSocket 唔穩定）→ **直接 IP（http://116.206.150.233:80）穩定**（冇斷線）— 🔴 **未解決：account balance 未顯示**（agent 有 Registered + 穩定連線 — 但上報 account_info 可能 server 未寫 DB — 要查 handle_sync + VPS DB agent 表） | 🟡 進行中 | VPS 搬遷完成度 ~80%（網頁 serve + agent 連線 OK — balance 顯示待修） |
