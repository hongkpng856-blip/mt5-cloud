...
| # | 描述 | 狀態 | 影響 |
|---|---|---|---|
| 170 | **Git watcher 自動 restart server 閃退（未修）** — git_auto_update.py 每 60 秒 check GitHub + pull 成功（code 更新）但 `restart_server()` 用 Python Popen（CREATE_NEW_CONSOLE）開 server 喺 VPS 閃退（verify 10061 — server 冇起到）— **手動開 server work**（PowerShell 設 env — `$env:RENDER="1";$env:PORT="80";python server\app.py` — 唔設 env 會行 5000 → 網站 522）— **暫時方案：手動開 server**（cd `Desktop\VPS\tradotcom` → env → python）+ 手動 `git pull` 更新 — **待修：watcher Popen 改用同手動一樣方法（powershell -Command 設 env）或者只 pull 唔 restart** | 🟡 未修 | 暫時手動開 server — 全自動更新未完成 |
