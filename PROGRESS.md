...
| **v0.10.79** | 2026-08-26 | 📊 **Correlation Matrix 全面升級（用戶 3 問 → 3 改）** — ①**「名 + Magic Number」顯示**：matrix key 全部顯示「EA名 (magic)」（例如 `Breakout (240701)`）— 雙層對應：精確 (magic,symbol) match + **fallback 淨 magic match**（v0.10.78 行內人 magic=EA 身份 — 歷史舊 symbol 記錄都對應返 EA 名 — 根治「點解仲顯示 Magic#240701 raw」）②**Heatmap 色階優化**：漸變色（-1 深紅 → 0 灰 → +1 深綠 — hsl 漸變 + 背景 alpha）取代 4 檔硬色 + 色階 legend ③**散點圖**（Chart.js）：揀兩隻 EA → 每日 P&L 散點（server 加 `daily_pnl` 回傳）— 時間性：前端每 30 秒 refresh analysis（即時重算 Pearson） |
...
| 122 | **🔥 Correlation Matrix 顯示 raw Magic#（唔顯示 EA 名 — 用戶實測）** | agent.deals（MT5 歷史 deals）有舊 (magic, symbol) 記錄（240701+USDJPY 等）— config 而家 symbol 轉咗（AMD）→ `ea_name_by_key` 精確 (magic,symbol) match 唔到 → fallback 顯示 raw `Magic#240701 (USDJPY)` | v0.10.79 雙層對應：精確 match + **fallback 淨 magic match**（v0.10.78 magic=EA 身份 — 搵 config 第一隻用該 magic 嘅 EA）。實測：`Magic#240701 (USDJPY)` → `Breakout (240701)` | 08-26 |
...
**Git HEAD**: `5235365`（master）— v0.10.79（Correlation Matrix 升級：名+Magic 顯示（magic fallback）+ Heatmap 漸變 + 散點圖）；TODO：數據注入選擇功能未實行（見 TODO 段）
